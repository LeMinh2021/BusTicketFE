...
namespace WebsiteBanVeXe.Controllers
{
    public class SiteController : Controller
    {
        ...
        public ActionResult CoachSearch(FormCollection fc, int? page)
        {
            ...
            DateTime ngaychay1 = DateTime.ParseExact(departure_date, "M/d/yyyy", CultureInfo.InvariantCulture);
                //định dạng sang mm/dd/yy
            string ngaychay2 = ngaychay1.ToString("dd-MM-yyyy");
            ...
            if (typeTicket.Equals("enable"))
            {
                ...
                DateTime ngayden1 = DateTime.ParseExact(ngayve, "M/d/yyyy", CultureInfo.InvariantCulture);
                string ngayden2 = ngayden1.ToString("dd-MM-yyyy");
                DateTime ngayden3 = DateTime.Parse(ngayden2);
                //In ngày ra view
                ViewBag.ngayden = ngayve;
                ViewBag.date = ngayden3.ToString("dd-MM-yyyy");
            ...
            else
            {
                ...
                var date = db.tickets.Where(m => m.status == 1).FirstOrDefault();
                //ve 1 chieu
                var list = db.tickets.Where(m => m.departure_address.Contains(departure_address) && m.arrival_address.Contains(arrival_address)).
             Where(m => m.day == ngaychay3).ToList();
